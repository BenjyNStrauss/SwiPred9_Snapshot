'''
Edited by Benjamin Strauss (2024)
Tokenization import is commented so how the language is done can be traced
...
def run_pretraining(create_model_function, epoch_generator, h5_dataset_file_path, create_model_kwargs = {}, optimizer_class = keras.optimizers.Adam, lr = 2e-04, \
    ...
    model_generator = PretrainingModelGenerator(create_model_function, n_annotations, create_model_kwargs = create_model_kwargs, \
                    optimizer_class = optimizer_class, lr = lr, other_optimizer_kwargs = other_optimizer_kwargs, \
    ...
    with h5py.File(h5_dataset_file_path, 'r') as h5f:
        if not 'test_set_mask' in h5f.keys():
            raise ValueError("test_set_mask not found!")
...
class ModelTrainer:
    ...
    def setup(self, dataset_handler, resume_from = None):
        ...
        #print(starting_episode.seq_len)
    ...
    def train_next_epoch(self, autosave = True):
        ...
        if DEBUG:
            print("changed_episode: " + str(changed_episode))
        
        X, Y, sample_weights = self.epoch_generator.create_next_epoch()
        
        if assertNoNegatives(X) or assertNoNegatives(Y):
            raise ValueError("X or Y has negative value")
        
        log('Epoch %d (current sample %d):' % (self.current_epoch_index, self.epoch_generator.current_sample_index))
        
        if DEBUG:
            sp_debug_log("=========================================================\n");
            sp_debug_log("current_epoch_index:  "+str(self.current_epoch_index) +"\n");
            sp_debug_log("current_sample_index: "+str(self.epoch_generator.current_sample_index) +"\n");
            sp_debug_log("fit_callbacks:        "+str(self.fit_callbacks) +"\n");
            sp_debug_log("episode               "+str(episode) +"\n")
            sp_debug_log("changed_episode       "+str(changed_episode) +"\n")
            sp_debug_log("batch_size:           "+str(episode.batch_size) +"\n");
            sp_debug_log("sample_weights:       "+str(sample_weights) +"\n");
            sp_debug_log("self.model:           "+str(self.model) +"\n");
            sp_debug_log("X:                    "+str(X) +"\n");
            sp_debug_log("Y:                    "+str(Y) +"\n");
            sp_debug_log("len(X):               "+str(len(X)) +"\n");
            sp_debug_log("len(Y):               "+str(len(Y)) +"\n");
...
class EpochGenerator:
    ...
    def create_next_epoch_Y(self):
        ...
        print("self._current_episode: "+str(self._current_episode))
    ...
    def _encode_epoch(self, encoded_seqs, encoded_annotation_masks):
        ...
        X = [noisy_encoded_seqs, noisy_annotation_masks.astype(np.int16)]
        Y = [np.expand_dims(encoded_seqs, axis = -1), encoded_annotation_masks.astype(np.int16)]
...
class EpisodeDataManager:
    ...
    def encode_next_epoch(self, log_length_dist = True):
        ...
        if log_length_dist:
            #note iteritems changed to items by Benjamin Strauss
            log('Epoch sequence length distribution (for seq_len = %d): %s' % (self.seq_len, \
                    ', '.join('%s: %s' % item for item in pd.Series(seq_lengths).describe().items())))
        
        #check for negative values
        assertNoNegatives(encoded_seqs)
    ...
    def _encode_epoch(self, epoch_sample_cache):
        ...
        encoded_seqs_pre_1 = [seq_tokens + max(self.seq_len - len(seq_tokens), 0) * [pad_token_index] for seq_tokens in \
                trimmed_tokenized_seqs]
        
        #assertNoNegatives(encoded_seqs_pre_1)
        #print(encoded_seqs_pre_1)
        
        #Issue HERE
        encoded_seqs = np.array(encoded_seqs_pre_1).astype(np.int16)
