...
public class SubstitutionMatrixHelper implements Serializable {
	...
	// reads in a substitution matrix from a resource file  TODO: edited by benjy
	private static InputStreamReader getReader(String file) {
		String resourcePathPrefix = "files/tools/biojava/";
		String inputStreamName = String.format("%s.txt",resourcePathPrefix+file);
		//BaseTools.qp(inputStreamName);
		FileInputStream myInputStream = null;
		//myInputStream = (InputStream) SubstitutionMatrixHelper.class.getResourceAsStream(inputStreamName);
		try {
			myInputStream = new FileInputStream(inputStreamName);
		} catch (FileNotFoundException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
		
		//BaseTools.qp(myInputStream);
		
		return new InputStreamReader(myInputStream);
	}
	
	/* // reads in a substitution matrix from a resource file
	private static InputStreamReader getReader(String file) {
		String resourcePathPrefix = "matrices/";
		InputStream inputStreamReaderMeta = (InputStream) SubstitutionMatrixHelper.class.getResourceAsStream(String.format("/%s.txt",
				resourcePathPrefix+file));
		
		//BaseTools.qp(inputStreamReaderMeta);
		return new InputStreamReader(inputStreamReaderMeta);
	}*/
