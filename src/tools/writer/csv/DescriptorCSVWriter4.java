 ...
 * Version 4.0 of CSVWriter, designed to be able to write customizable CSVs
 * 
 * TODO: writing encodings needs an upgrade...
 * 
...
public class DescriptorCSVWriter4 extends AbstractDescriptorCSVWriter {
	...
	private String formLine(AminoChain<?> chain, int index) {
		...
		for(Metric col: this) {
			...
			if(col instanceof Descriptor) {
			...
			} else if (col instanceof EncodingType) {
				/*
				 * TODO: this is a solution in beta
				 * currently we are unsure of a better solution
				 */
				double[] encoding = chain.get(index).getEncoding((EncodingType) col);
				if(encoding == null) { 
					for(int ii = 0; ii < ((EncodingType) col).encodingLength(); ++ii) {
						lineBuilder.append("NaN,");
					}
				} else {
				...
				lineBuilder.append("unknown-metric");
			}
			lineBuilder.append(",");
		}
		trimLastChar(lineBuilder);
	...
	private String makeHeader() {
		...
		for(Metric type: this) {
			if(type instanceof EncodingType) {
				EncodingType eType = (EncodingType) type;
				for(int ii = 0; ii < eType.encodingLength(); ++ii) {
					headerBuilder.append(type +"-v"+ii+",");
				}
			} else {
