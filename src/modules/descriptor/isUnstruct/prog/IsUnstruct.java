...
/**
 * Runs IsUnstruct: an algorithm for calculating disorder propensity from residue sequence
 * Translated from original C code by Lobanov and Galzitskaya
 * .ius not working (is this still true as of 6/21/19?)
 * @translator Benjy Strauss
 * 
 * note: we only care about the ACIDP.plp values
 *
 */
...
public class IsUnstruct extends PredDel {
	...
	/**
	 * New method by Benjy Strauss for SwitchPredict
	 * @return: isUnstruct values for all chains in the sequence
	 */
	public static double[] getPLPs(String sequence) {
		if(sequence.length() == 0) { throw new EmptyChainException("Cannot run IsUnstruct on an empty chain!"); }
		SEQ_AC sa = new SEQ_AC();
		sa.LoadFasta(sequence);
		
		PredDel pred = new PredDel();
		pred.Predict(sa);
		
		double retval[] = new double[sa.ac.length];
		
		for(int index = 0; index < sa.ac.length; ++index) {
			retval[index] = sa.ac[index].plp;
		}
