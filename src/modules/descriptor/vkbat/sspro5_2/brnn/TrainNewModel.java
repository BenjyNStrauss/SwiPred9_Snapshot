...
public class TrainNewModel extends CppTranslator {
	...
	static void train_model(Options OPT, Model BRNN, Dataset TRAIN, Dataset TEST, String fileout) {
		// Initial Model Evaluation
		qp("Testing initial model on train dataset");
		test_model(BRNN,TRAIN);
		qp("Testing initial model on test dataset");
		...
		// Training iterations
		qp("Ready for training the BRNN model...\n\n");
		for(int epoch=0; epoch<num_epochs; epoch++) {
			...
			if(shuffle != 0){
				...
				for(int i=0;i<n;i++){
					...
					int q=(int) (rand()%n);
					int t=train_seq[p];
					train_seq[p]=train_seq[q]; train_seq[q]=t;
				}
			}
			...
			for(int i=0;i<num_seq;i++) {
				...
				BRNN.expectation(seq); num_pos_batch+=seq.Num_Positions;
			...
			if(current_error<previous_error) {
				...
				BRNN.write(fileout); previous_error=current_error; adap_num_epochs=0;
	...
	public static void main(String[] args) {
		// Script Arguments
		String usage = " options_file train_dataset test_dataset output_model\n\n";
		if(args.length != 5){
			throw new SSproExitException("\n  Script usage : " + args[0] + usage);
		}

		// BRNN & Training Options
		qp("\n\nReading options to train the BRNN...\n");
		...
		Dataset train=new Dataset(); train.load(args[2]);
		...
		qp("Generating random initial model...\n");
		Model brnn=new Model();
		brnn.initialize(opt.FEATURES,
		opt.CLASSES,opt.HIDDEN,opt.CONTEXT_FWD,opt.CONTEXT_BWD,
		opt.OUTPUTS_FWD,opt.OUTPUTS_BWD,opt.HIDDEN_FWD,opt.HIDDEN_BWD);
		qp("Writing random initial model in \"" + args[4]);
		qp("\"...\n\n");
		brnn.write(args[4]);

		// Data Compatibility
		int c=1;
		if(train.Num_Features!=test.Num_Features){ c=0; }
		...
		// Train Model
		train_model(opt,brnn,train,test,args[4]);
	}
}
