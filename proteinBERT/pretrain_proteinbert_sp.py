'''
Benjamin Strauss's SwiPred ProteinBERT Pretrainer
(To be) modified from the original

Should crash when parsing tokens…
...
from proteinBERT.save_file_manager import SAVE_PATH, SPBertSaveFile

from proteinBERT import save_file_manager

model_types = data_sets = {  "p", "ps3", "s8", "s3", "pbert", "ps8"  }
...
if __name__ == '__main__':
    ...
    for REDUNDANCY in ["nr", "wr"]:
        for DATABASE in ["pdb", "unp"]:
            #if not os.path.exists(SAVE_PATH+REDUNDANCY):
            #    os.makedirs(SAVE_PATH+REDUNDANCY+"-"+DATABASE)
            
            for model_type in model_types:
                
                # ps3_epoch_196_dssp_nr
                if (REDUNDANCY != "nr" or DATABASE != "pdb" or model_type != "ps3"):
                    continue
                
                print("Making Model: " + model_type + "("+REDUNDANCY+"-"+DATABASE+")")
                default_input_path = "./input/swipredbert/"+REDUNDANCY+"/"+model_type+"token-"+DATABASE+".h5";
                default_autosave_dir = SAVE_PATH
                
                save_file_manager.GLOBAL_ACTIVE_SAVE_FILE = SPBertSaveFile(key=model_type, database=DATABASE, epochs=None, redundancy=REDUNDANCY)
                save_file_manager.test_GLOBAL_ACTIVE_SAVE_FILE()
                #+REDUNDANCY+"-"+DATABASE+"/"+model_type
                ...
                parser.add_argument('--dataset-file', dest = 'dataset_file', metavar = '/path/to/dataset.h5', type = get_parser_file_type(parser, must_exist = True), \
                        default = default_input_path, help = 'The dataset h5 file with the protein sequences and annotations to train on.')
                parser.add_argument('--autosave-dir', dest = 'autosave_dir', metavar = '/path/to/autosave_dir/', type = get_parser_directory_type(parser, create_if_not_exists = True), \
                        default = default_autosave_dir, help = 'The directory to save the model state into throughout the training process.')
                ...
                #every_saves_to_keep = args.keep_every_autosaves
                save_every = 4;
                autosave_manager = AutoSaveManager(args.autosave_dir, every_epochs_to_save = save_every, every_saves_to_keep = save_every)
                ...
                if args.resume_from is None:
                    ...
                    print("pretrain-proteibert-sp: needs edits")
                ...
                run_pretraining(create_model_function, epoch_generator, args.dataset_file, create_model_kwargs = create_model_kwargs, optimizer_class = optimizer_class, lr = args.lr, \
                        ...
                        n_epochs = num_epochs, fit_callbacks = fit_callbacks)
                log('Done.')