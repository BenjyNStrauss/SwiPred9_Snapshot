...
def create_h5_testset(maskLine: str, outfile: str):
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser("swipred_dataset_parser")
    parser.add_argument("ps8token",   help="Primary+8-Class Secondary Structure input file.", type=str)
    parser.add_argument("ps3token",   help="Primary+3-Class Secondary Structure input file.", type=str)
    parser.add_argument("ptoken",     help="Primary Structure input file.",                   type=str)
    parser.add_argument("s8token",    help="8-Class Secondary Structure input file.",         type=str)
    parser.add_argument("s3token",    help="3-Class Secondary Structure input file.",         type=str)
    parser.add_argument("pberttoken", help="ProteinBERT token input file.",                   type=str)
    parser.add_argument("testmask",   help="Binary string determining which sequences go in the test set.", type=str)
    
    create_h5_dataset(str(parser.parse_args().ps8token)+".txt",   str(parser.parse_args().ps8token)+".h5", 6);
    create_h5_dataset(parser.ps3token+".txt",   parser.ps3token+".h5", 6);
    create_h5_dataset(parser.ptoken+".txt",     parser.ptoken+".h5", 4);
    create_h5_dataset(parser.s8token+".txt",    parser.s8token+".h5", 4);
    create_h5_dataset(parser.s3token+".txt",    parser.s3token+".h5", 4);
    create_h5_dataset(parser.pberttoken+".txt", parser.pberttoken+".h5", 4);
    
    create_h5_testset(parser.testmask, str(parser.ps8token)+".h5");
    create_h5_testset(parser.testmask, parser.ps3token+".h5");
    create_h5_testset(parser.testmask, parser.ptoken+".h5");
    create_h5_testset(parser.testmask, parser.s8token+".h5");
    create_h5_testset(parser.testmask, parser.s3token+".h5");
    create_h5_testset(parser.testmask, parser.pberttoken+".h5");
